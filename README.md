...
Keys: `→ space j` next · `← k` previous · `home` `end` · `e` edit · `i` invert
the deck · `?` help. None of them fire while the editor has focus.
...
| `spacing(deg?)` | read or set the deck-wide max hue spacing (slide 12's slider) |
| `shuffled(on?)` | read or set the deck-wide pick mode: first few of the ring, or a random subset (slide 13's toggle) |
...
| `rgb(color)` · `luma(color)` · `grey(color)` | measured through a 1px canvas |
...
three hues you pick on slide 12 are the same three that get stretched, named
...
Drop files in `public/`; reference them as `/name.png`. Slide 3 uses
